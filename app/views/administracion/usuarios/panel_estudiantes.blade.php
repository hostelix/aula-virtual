				...
				<div class="tabs">
					<div id="tab-2" class="tab">
						<article>
							...
							<hr>
							<div>
							&nbsp;&nbsp;&nbsp;<a href="/Administracion/crearestudiante" class="btn btn-success">Crear nuevo estudiante</a><br>
							<hr>
								<table class="table table-striped">
						            <thead>
						                <tr>
							                <th>ID</th><th>NOMBRES</th><th>APELLIDOS</th>
							                <th>CEDULA</th><th>USUARIO</th><th>TELF MOVIL</th><th>CREADO</th>
							                <th>ACCION</th
						                </tr>
						            </thead>
						            <tbody>
						            	@if(count($estud) != 0)
						            		@foreach($estud as $est)
								                <tr class="info">
								                  	<td>{{$est->id}}</td><td>{{$est->nombres}}</td><td>{{$est->apellidos}}</td><td>{{$est->cedula}}</td>
								                  	<td>{{$est->usuario->usuario}}</td><td>{{$est->telf_movil}}</td><td>{{$est->created_at}}</td>
								                  	<td>{{HTML::link('/Administracion/detallesestudiante/'.$est->id,'Ver',array('class'=>"btn btn-success"))}}
								                </tr>
								            @endforeach
								        @else
									        <ul class="states">
												<li class="error">En este momento no hay estudiantes registrados</span></li>
											</ul>
								        @endif
						            </tbody>
						       	</table>
